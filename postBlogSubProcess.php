<?php
	...
	$blogUserID		=	filter_has_var(INPUT_POST, 'userID') ? $_POST['userID']: null;
	...
	$blogUserID		= filter_var($blogUserID,FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES);
	$blogUserID 	= filter_var($blogUserID,FILTER_SANITIZE_SPECIAL_CHARS);
  ...
  if(isset($_SESSION['userID'])){
		if ($accountID == "1") {	
			...
			if (mysqli_affected_rows($conn)>0){ 
				$errorList[] = "Sorry blog is already posted.";
			} 
			...
			if (!empty($errorList)) {
				...
				$sqlDelSub = "DELETE FROM submitblogpost 
				...
				if(mysqli_affected_rows($conn)>0){
					echo "<p>Submitted blog successfully submitted.</p>";
				}
				else {
					echo "<p style=\"color: red\">Unable to delete submitted blog!</p>";
				}
			}
